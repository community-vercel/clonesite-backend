...
router.get('/:id', protect, async (req, res) => {
  try {
    const request = await Request.findById(req.params.id)
      .populate('customer', 'firstName lastName avatar rating address isVerified phone email')
      .populate('category', 'name slug icon customFields')
      .populate('subCategory', 'name slug')
      .populate('selectedProvider', 'firstName lastName avatar businessName rating')
      .populate('quotes.provider', 'firstName lastName avatar businessName rating isVerified');
    ...
    const isProvider = request.quotes.some(quote => quote.provider._id.toString() === req.user.id);
    const isSelectedProvider = request.selectedProvider && 
      request.selectedProvider._id.toString() === req.user.id;

    if (!isCustomer && !isProvider && !isSelectedProvider) {
    ...
    if (!isCustomer && !isSelectedProvider) {
      // For other providers, hide customer contact info and other quotes
      responseData.customer.phone = undefined;
      responseData.customer.email = undefined;
      responseData.quotes = responseData.quotes.filter(
        quote => quote.provider._id.toString() === req.user.id
      );
    }
...
router.post('/', protect, requireCustomer, multerUpload.array('attachments', 5), [
  ...
  body('timeline.flexibility').optional().isIn(['asap', 'within_week', 'within_month', 'flexible']),
  body('timeline.urgency').optional().isIn(['low', 'medium', 'high', 'urgent']),
